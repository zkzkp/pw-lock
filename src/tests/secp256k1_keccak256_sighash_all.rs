...
fn gen_tx_with_grouped_args<R: Rng>(
    ...
    let sighash_all_cell = CellOutput::new_builder()
        .capacity(
            Capacity::bytes(KECCAK256_ALL_BIN.len())
                .expect("script capacity")
                .pack(),
        )
        .build();
    let sighash_all_cell_data_hash = CellOutput::calc_data_hash(&KECCAK256_ALL_BIN);
    dummy.cells.insert(
        sighash_all_out_point.clone(),
        (sighash_all_cell, KECCAK256_ALL_BIN.clone()),
    );
